...
class MapGenerator:
    ...
    def make_skeleton(self, method="dfs"):
        map = None
        while count_passages_to_walls_ratio(map) <= 0.8:
            if method == 'prim':
                print("Using Prim's algorithm")
                maze = self.generate_thin_maze_prim()
            elif method == 'ellers':
                print("Using Ellers's algorithm")
                maze = self.generate_thin_maze_ellers()
            else:
    ...
    def generate_thin_maze(self):
        ...
        start_cell = grid[0]
        self._generate_thin_maze_recursive(start_cell, grid, self.width, self.height)

        return grid

    def _generate_thin_maze_recursive(self, current_cell, grid, width, height):
        current_cell.visited = True
        
        next_cell = current_cell.get_next(grid, width, height)
        
        while next_cell is not None:
            remove_wall(current_cell, next_cell)
            
            self._generate_thin_maze_recursive(next_cell, grid, width, height)
            
            next_cell = current_cell.get_next(grid, width, height)

    def generate_thin_maze_ellers(self):
        return []
    
    def generate_thin_maze_prim(self):
        """
        Генерирует лабиринт используя алгоритм Прима (Prim's Algorithm)
        Создает более равномерно распределенные пути по сравнению с DFS
        """
        grid = []
        for i in range(self.height):
            for j in range(self.width):
        ...
        start_cell = grid[start_index]
        start_cell.visited = True
        
        # Frontier - список граничных клеток (соседи уже посещенных клеток)
        frontier = []
        
        # Добавляем соседей стартовой клетки в frontier
        def add_to_frontier(cell, from_cell):
            neighbors = []
            neighbor_up_index = get_index(cell.i - 1, cell.j, self.width, self.height)
            neighbor_right_index = get_index(cell.i, cell.j + 1, self.width, self.height)
            neighbor_down_index = get_index(cell.i + 1, cell.j, self.width, self.height)
            neighbor_left_index = get_index(cell.i, cell.j - 1, self.width, self.height)
            
            for idx in [neighbor_up_index, neighbor_right_index, neighbor_down_index, neighbor_left_index]:
                if idx >= 0 and not grid[idx].visited:
                    neighbors.append((grid[idx], from_cell))
            
            return neighbors
        
        # Инициализируем frontier
        for neighbor, from_cell in add_to_frontier(start_cell, start_cell):
            if neighbor not in [f[0] for f in frontier]:
                frontier.append((neighbor, from_cell))
        
        # Пока есть клетки в frontier
        while frontier:
            # Выбираем случайную клетку из frontier
            rand_index = random.randint(0, len(frontier) - 1)
            current_cell, from_cell = frontier.pop(rand_index)
            
            # Если клетка еще не посещена
            if not current_cell.visited:
                current_cell.visited = True
                # Удаляем стену между текущей клеткой и клеткой, из которой пришли
                remove_wall(current_cell, from_cell)
                
                # Добавляем соседей текущей клетки в frontier
                for neighbor, _ in add_to_frontier(current_cell, current_cell):
                    # Проверяем, что сосед еще не в frontier
                    if neighbor not in [f[0] for f in frontier]:

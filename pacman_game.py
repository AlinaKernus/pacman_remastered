...
SCREEN_TILE_SIZE_WIDTH = 23  # Квадратный экран игры
...
# Инициализация pygame (если еще не инициализирован)
if not pygame.get_init():
    pygame.mixer.pre_init(22050, -16, 1, 1024)
    pygame.init()
else:
    # Если pygame уже инициализирован, только настраиваем mixer
    pygame.mixer.pre_init(22050, -16, 1, 1024)
    pygame.mixer.init()
...
# Инициализируем pygame.display для загрузки изображений
# Создаем временное минимальное окно (не будет использоваться)
# Реальное окно будет передано извне через run_game_loop
try:
    if not pygame.display.get_init():
        pygame.display.init()
        # Создаем минимальное окно для инициализации display
        # Это окно не будет использоваться, так как мы используем переданный surface
        temp_window = pygame.display.set_mode((1, 1))
except:
    # Если display уже инициализирован, просто продолжаем
    pass
screen = None  # Будет установлен в run_game_loop
...
class game:
    ...
    def DrawScore(self, show_score=True):
        global rect_list
        # Все элементы интерфейса теперь отображаются вне игры
        # Оставляем только сообщения игры (game over, ready)
        if self.mode == 3:
            # Отображаем "Game over" красным текстом вместо изображения
            font = pygame.font.Font(None, 48)
            game_over_text = font.render("Game over", True, (255, 0, 0))  # Красный цвет
            text_rect = game_over_text.get_rect()
            screen.blit(game_over_text, (self.screenSize[0] / 2 - text_rect.width / 2,
                                         self.screenSize[1] / 2 - text_rect.height / 2))
        ...
        for row in range(-1, thisGame.screenTileSize[0] + 1, 1):
            for col in range(-1, thisGame.screenTileSize[1] + 1, 1):
                ...
                if useTile != 0 and useTile != tileID['door-h'] and useTile != tileID['door-v']:
                    ...
                    # High scores убран из игры
                    elif useTile == tileID['hiscores']:
                        # Не отображаем high scores
                        pass
...
# Функция CheckIfCloseButton удалена - обработка событий будет в вызывающем коде
...
def CheckInputs():
    ...
    # ESC и обработка game over удалены - управление будет извне
...
# Окно не создается здесь - оно будет передано извне
# Инициализация джойстика будет выполнена при первом запуске игры
js = None
...
def init_joystick():
    """Инициализирует джойстик при первом запуске"""
    global js
    if js is None:
...
def run_game_loop(screen_surface):
    """
    Основной цикл игры. Обновляет состояние игры и отрисовывает на переданный surface.
    Возвращает строку состояния: 'playing', 'game_over', 'level_complete'
    """
    global js, oldEdgeLightColor, oldEdgeShadowColor, oldFillColor, screen
    
    # Устанавливаем глобальный screen для использования в функциях Draw
    screen = screen_surface
    
    # Инициализация джойстика при первом запуске
    init_joystick()
    ...
    if thisGame.mode == 1:
        ...
        if thisGame.modeTimer == 60:
            ...
            thisGame.lives -= 1
            # Предотвращаем отрицательные жизни
            if thisGame.lives < 0:
                thisGame.lives = 0
            if thisGame.lives <= 0:
                thisGame.SetMode(3)
        ...
        # game over - просто отрисовываем, не обрабатываем ввод для новой игры
        pass
    ...
    # Используем глобальный screen (установлен в начале функции)
    ...
    thisGame.DrawScore(show_score=False)  # Счет отображается вне игры

    del rect_list[:]

    # Определяем состояние игры для возврата
    if thisGame.mode == 3:
        return 'game_over'
    elif thisGame.mode == 6 or thisGame.mode == 7 or thisGame.mode == 10:
...
def start_new_game():
    """Инициализирует новую игру. Вызывается перед первым запуском run_game_loop"""
...
def get_lives():
    ...
    return thisGame.lives


def get_level():
    """Возвращает текущий уровень"""
    return thisGame.levelNum
